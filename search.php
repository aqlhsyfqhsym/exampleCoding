...
<!-- ========================================end user======================= -->
	...
	<div id="stafflist">
        ...
        <h1> <br><br>&nbsp;&nbsp; CUSTOMER BOOKING TICKET <br><sub> &nbsp;&nbsp;&nbsp;&nbsp; LIST OF CUSTOMER BOOKING TICKET </sub> </h1>
<?php
include_once('db.php');
if(isset($_POST['search'])){
	$q = $_POST['q'];
	$query = mysqli_query($mysqli,"SELECT * FROM bookingts WHERE `bpark` LIKE '%$q%'"); 
//Replace table_name with your table name and `thing_to_search` with the column you want to search
	...
	if($count == "0"){
		...
		echo "<table border=2 cellpadding=5 cellspacing=2 align=center>
         ...
         <tr><th colspan=6> CUSTOMER DETAILS </th>
		...
		$s = $row['bpark']; // Replace column_to_display with the column you want the results from
			 print '<tr>
                 <td>' . $row["bookID"]. '</td> 
                 <td>' . $row["bIC"]. '</td>
                 <td>' . $row["bemail"]. '</td> 
